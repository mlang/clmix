...
[[nodiscard]] mix_result build_mix(
  const vector<track_info>& tracks,
  double bpm,
...
void write_cuesheet(path const &cue_path,
  path const &out_path, std::vector<mix_cue> const &cuepoints
)
{
  std::ofstream sheet(cue_path, std::ios::trunc);
  if (!sheet) {
  ...
  {
    assert(t_sec >= 0);
    auto total_cd_frames = static_cast<int>(std::round(t_sec * 75.0));

    return std::format("{:02d}:{:02d}:{:02d}",
      total_cd_frames / (75 * 60),
      (total_cd_frames / 75) % 60,
      total_cd_frames % 75
    );
  };

  auto escape = [](std::string_view s) -> string
  {
    std::string out;
    out.reserve(s.size());
    for (char c : s) {
  ...
  println(sheet, "REM Generated by clmix");
  println(sheet, "FILE \"{}\" WAVE", out_path.filename().generic_string());

  // Always create TRACK 01 starting at 00:00:00
  if (!cuepoints.empty()) {
    println(sheet, "  TRACK 01 AUDIO");
    auto title = cuepoints.front().track.filename().stem().generic_string();
    println(sheet, "    TITLE \"{}\"", escape(title));
  }
  println(sheet, "    INDEX 01 00:00:00");

  // For each internal cuepoint (skip first and last), create a new TRACK
  // starting at that cue's time.
  if (cuepoints.size() > 2) {
    auto cues = views::drop(views::enumerate(cuepoints), 1);
...
)
{
  ...
  write_cuesheet(cue_path, out_path, mix.cues);
